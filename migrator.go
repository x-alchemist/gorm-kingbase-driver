...
func (m Migrator) ColumnTypes(value interface{}) (columnTypes []gorm.ColumnType, err error) {
	...
	err = m.RunWithValue(value, func(stmt *gorm.Statement) error {
		var (
			...
			columns, err         = m.queryRaw( //原sql:"SELECT c.column_name, c.is_nullable = 'YES', c.udt_name, c.character_maximum_length, c.numeric_precision, c.numeric_precision_radix, c.numeric_scale, c.datetime_precision, 8 * typlen, c.column_default, pd.description, c.identity_increment FROM information_schema.columns AS c JOIN pg_type AS pgt ON c.udt_name = pgt.typname LEFT JOIN pg_catalog.pg_description as pd ON pd.objsubid = c.ordinal_position AND pd.objoid = (SELECT oid FROM pg_catalog.pg_class WHERE relname = c.table_name AND relnamespace = (SELECT oid FROM pg_catalog.pg_namespace WHERE nspname = c.table_schema)) where table_catalog = ? AND table_schema = ? AND table_name = ?",
				"SELECT c.column_name, c.is_nullable = 'YES', "+
					"c.udt_name, c.character_maximum_length, "+
					"c.numeric_precision, c.numeric_precision_radix, "+
					"c.numeric_scale, c.datetime_precision, 8 * c.typlen, "+
					"c.column_default, c.description, c.identity_increment "+
					"FROM (select current_database() as table_catalog, "+
					"r.relnamespace::regnamespace::name as table_schema, "+
					"r.relname as table_name, t.typlen as typlen, "+
					...
					"cast(case t.typtype when 'd' then t.typbasetype::regtype::name else t.typname end as information_schema.sql_identifier) as udt_name, "+
					"cast(information_schema._pg_char_max_length(information_schema._pg_truetypid(a, t), information_schema._pg_truetypmod(a, t)) "+
					"as information_schema.cardinal_number) as character_maximum_length, "+
					"cast(case when length(a.attgenerated) = 0 then pg_get_expr(ad.adbin, ad.adrelid) end as information_schema.character_data) as column_default, "+
					"cast(information_schema._pg_numeric_precision(information_schema._pg_truetypid(a, t), information_schema._pg_truetypmod(a, t)) "+
					"as information_schema.cardinal_number) as numeric_precision, "+
					"cast(information_schema._pg_numeric_precision_radix(information_schema._pg_truetypid(a, t), information_schema._pg_truetypmod(a, t)) "+
					"as information_schema.cardinal_number) as numeric_precision_radix, "+
					"cast(information_schema._pg_numeric_scale(information_schema._pg_truetypid(a, t), information_schema._pg_truetypmod(a, t)) "+
					"as information_schema.cardinal_number) as numeric_scale, "+
					"cast(information_schema._pg_datetime_precision(information_schema._pg_truetypid(a, t), information_schema._pg_truetypmod(a, t)) "+
					"as information_schema.cardinal_number) as datetime_precision, "+
					"cast(seq.seqincrement as information_schema.character_data) as identity_increment, "+
					"pd.description as description "+
					"from pg_attribute a join pg_type t on (a.atttypid = t.oid and a.attnum > 0) "+
					"join _rel r on (a.attrelid = r.oid and a.attnum > 0) "+
					"left join _attdef ad on (a.attrelid = ad.adrelid and a.attnum = ad.adnum) "+
					"left join pg_catalog._desc pd ON (pd.objsubid = a.attnum and pd.objoid = r.oid) "+
					"left join (_dep dep join _seq seq on (dep.classid = '_rel'::regclass and dep.objid = seq.seqrelid and dep.deptype = 'i')) "+
					"on (dep.refclassid = '_rel'::regclass and dep.refobjid = r.oid and dep.refobjsubid = a.attnum and a.attidentity != '')) AS c "+
